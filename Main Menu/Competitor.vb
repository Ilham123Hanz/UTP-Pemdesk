...
Public Class Competitor
    ...
    ' --- ISI LIST TIM DI SEBELAH KIRI ---
    Public Sub RefreshTeamList()
        listTeams.Items.Clear()
        listTeams.Items.Add("-- Semua Tim --")
            ...
            If TeamEntry.dgvTeams.Rows.Count > 0 Then
                For Each row As DataGridViewRow In TeamEntry.dgvTeams.Rows
                    If Not row.IsNewRow Then
                        ...
                        If teamName <> "" AndAlso Not listTeams.Items.Contains(teamName) Then
                            listTeams.Items.Add(teamName)
                        End If
                    End If
                Next
            End If
        ...
        If listTeams.Items.Count = 1 Then
            listTeams.Items.Add("Elang Biru")
            listTeams.Items.Add("Naga Hitam")
            listTeams.Items.Add("Garuda Sakti")
            listTeams.Items.Add("Harimau Putih")
            listTeams.Items.Add("Dojo Rajawali")
        End If

        listTeams.SelectedIndex = 0
    End Sub

    ' --- FILTER SAAT KLIK NAMA TIM DI KIRI ---
    Private Sub listTeams_SelectedIndexChanged(sender As Object, e As EventArgs) Handles listTeams.SelectedIndexChanged
        Dim selectedTeam As String = If(listTeams.SelectedItem IsNot Nothing, listTeams.SelectedItem.ToString(), "")
        If selectedTeam = "" Then Return

        lstTeams.CurrentCell = Nothing

        For Each row As DataGridViewRow In lstTeams.Rows
            If row.IsNewRow Then Continue For
            Dim teamVal As String = If(row.Cells(4).Value IsNot Nothing, row.Cells(4).Value.ToString(), "")
            If selectedTeam = "-- Semua Tim --" Then
                row.Visible = True
            Else
                row.Visible = teamVal.Contains(selectedTeam)
            End If
        Next
    End Sub

    ' --- DROPDOWN OTOMATIS TERBUKA SAAT DIKLIK ---
    Private Sub cmbTeam_Click(sender As Object, e As EventArgs) Handles cmbTeam.Click
        cmbTeam.DroppedDown = True
    End Sub
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        ...
        lstTeams.Rows.Add(lstTeams.Rows.Count + 1, "❌", "📝", txtName.Text, cmbTeam.Text, txtTeamInfo.Text, imgThumb)
            ...
            Try
                ...
                For row As Integer = 2 To xlRange.Rows.Count
                    ...
                    If Not String.IsNullOrEmpty(nameVal) Then
                        lstTeams.Rows.Add(lstTeams.Rows.Count + 1, "❌", "📝", nameVal, teamVal, infoVal, Nothing)
                    End If
                Next
    ...
    Private Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        If lstTeams.Rows.Count = 0 Then Return
        Dim xlApp As New Excel.Application
        Try
            ...
            For i As Integer = 0 To lstTeams.Rows.Count - 1
                xlSheet.Cells(i + 2, 1) = lstTeams.Rows(i).Cells(3).Value
                xlSheet.Cells(i + 2, 2) = lstTeams.Rows(i).Cells(4).Value
                xlSheet.Cells(i + 2, 3) = lstTeams.Rows(i).Cells(5).Value
            Next
            xlApp.Visible = True
        Catch ex As Exception
    ...
    Private Sub UpdateTotal()
        lblTotalRecords.Text = "Total Records : " & lstTeams.Rows.Count
    End Sub
    ...
    Private Sub btnDeleteAll_Click(sender As Object, e As EventArgs) Handles btnDeleteAll.Click
        If MessageBox.Show("Hapus semua data?", "Konfirmasi", MessageBoxButtons.YesNo) = DialogResult.Yes Then
            lstTeams.Rows.Clear() : UpdateTotal()
        End If
    End Sub
    ...
    Private Sub ApplyRowColors()
        For i As Integer = 0 To lstTeams.Rows.Count - 1
            If i Mod 2 = 0 Then
                lstTeams.Rows(i).DefaultCellStyle.BackColor = Color.White
            Else
                lstTeams.Rows(i).DefaultCellStyle.BackColor = Color.FromArgb(255, 200, 200)
            End If
        Next
    End Sub
